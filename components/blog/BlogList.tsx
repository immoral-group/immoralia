...
export default function BlogList({ articulos }: { articulos: Articulo[] }) {
  ...
  return (
    <div ref={listRef} className="scroll-mt-24">
      ...
      {/* Disposición del grid principal y Sidebar */}
      <div className="max-w-7xl mx-auto px-6 mt-8">
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 md:gap-10">
          
          {/* Columna Izquierda: Rejilla de artículos (con orden-2 en móviles para ir debajo de los filtros) */}
          <div className="order-2 lg:order-1 lg:col-span-9">
            {!isFilteringOrSearching ? (
              ...
              <ArchiveSection
                articulos={currentArchiveArticles}
                title={trimmed.length > 0 ? "Resultados de búsqueda" : "Artículos filtrados"}
                countLabel={filteredAndSorted.length}
              />
            ) : (
          ...
          {/* Columna Derecha: Sidebar de Filtros (con orden-1 en móviles para ir arriba de los artículos) */}
          <div className="order-1 lg:order-2 lg:col-span-3">
            <div className="space-y-6 bg-white/70 backdrop-blur-md border border-[#3B80DF]/15 rounded-3xl p-6 shadow-sm lg:sticky lg:top-24">
              
              {/* Sección de Categorías */}
              <div>
                <h3 
                  className="text-black/80 flex items-center gap-2 mb-4 uppercase tracking-wider font-extrabold"
                  style={{ fontSize: '11px', fontFamily: 'Lexend, sans-serif' }}
                >
                  <ListFilter className="w-4 h-4 text-[#0077cc]" />
                  Categorías
                </h3>
                  ...
                  <button
                    ...
                    className={`w-full text-left px-3 py-1.5 rounded-xl text-xs transition-colors flex items-center justify-between cursor-pointer ${
                      selectedCategory === null
                        ? 'bg-[#0077cc]/10 text-[#0077cc] font-semibold'
              ...
              {/* Separador */}
              <div className="h-px bg-[#3B80DF]/10" />

              {/* Sección de Fecha (Calendario/Archivo) */}
              <div>
                <h3 
                  className="text-black/80 flex items-center gap-2 mb-4 uppercase tracking-wider font-extrabold"
                  style={{ fontSize: '11px', fontFamily: 'Lexend, sans-serif' }}
                ...
                <div className="space-y-3">
                  ...
                  <div className="flex flex-col gap-1.5">
                    <label className="text-[10px] text-black/40 font-medium" style={{ fontFamily: 'Lexend, sans-serif' }}>
                      Seleccionar Mes
                    </label>
                    <select
                      value={selectedMonth || ''}
                      onChange={(e) => setSelectedMonth(e.target.value || null)}
                      className="w-full bg-white border border-[#3B80DF]/15 rounded-xl px-3 py-2 text-xs outline-none focus:border-[#0077cc] text-black/80 transition-colors"
                      style={{ fontFamily: 'Lexend, sans-serif' }}
                    >
                  ...
                  {/* Selector de Día (Solo activo si hay mes seleccionado) */}
                  <div className="flex flex-col gap-1.5">
                    <label className="text-[10px] text-black/40 font-medium" style={{ fontFamily: 'Lexend, sans-serif' }}>
                      Seleccionar Día
                    </label>
                    <select
                      ...
                      className="w-full bg-white border border-[#3B80DF]/15 disabled:bg-slate-50 disabled:text-black/30 rounded-xl px-3 py-2 text-xs outline-none focus:border-[#0077cc] text-black/80 transition-colors"
                      style={{ fontFamily: 'Lexend, sans-serif' }}
                    >
  ...
  return (
    ...
    >
      <div className="relative overflow-hidden rounded-3xl border border-black/8 bg-white p-12 md:p-16 text-center shadow-sm">
        ...
        <button
          onClick={onClear}
          className="inline-flex items-center gap-2 px-5 py-3 rounded-xl border border-black/15 hover:border-[#0077cc]/50 text-black/80 hover:text-[#0077cc] transition-all duration-300 text-xs font-semibold cursor-pointer"
          style={{ fontFamily: 'Lexend, sans-serif' }}
        >
